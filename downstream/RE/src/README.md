...
# Calling our model requires setting a hyperparameter：--use_our_pretrain

```

export MODEL=KBioXLM_model
export MODEL_PATH=../../KBioXLM_model

task=GAD_hf_zh_en
datadir=../data/seqcls/$task
outdir=runs/$task/$MODEL-$LR-$EPOCH-$SEED
mkdir -p $outdir
...
### MODEL_PATH: Model Path ；task: Dataset name
