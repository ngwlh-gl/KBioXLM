...
from seqcls_model import seq_model
import datasets
import numpy as np
...
class ModelArguments:
    ...
    ours_pretrain_ckpt: Optional[str] = field(
        default=None,
        metadata={"help": "Where do you want to store the pretrained models downloaded from huggingface.co"},
    )
...
def main():
    ...
    # model=seq_model(model,len(label_to_id),config)
    ...
    if training_args.do_train:
        ...
        # 如果当前任务是bioasq，那么需要对数据进行1：1筛选
        # if 'bioasq' in data_args.train_file.lower():
        #     # 将index为0和index为1 的数据分别拿出来
        #     indexs_0=[]
        #     indexs_1=[]
        ...
        #         else:
        #             indexs_0.append(i)
        #     min_length=min(len(indexs_1),len(indexs_0))
        #     random.shuffle(indexs_0)
        #     random.shuffle(indexs_1)
        #     indexs_0=indexs_0[:min_length*2]
        #     indexs_1=indexs_1[:min_length*2]
        #     datasets_0=train_dataset.select(indices=indexs_0)
        #     datasets_1=train_dataset.select(indices=indexs_1)
        #     train_dataset = concatenate_datasets([datasets_0,datasets_1]).shuffle(seed=training_args.seed)
        #     train_dataset = concatenate_datasets([datasets_0,datasets_1]).shuffle(seed=training_args.seed)
    ...
    def compute_metrics(p: EvalPrediction, eval_dataset):
        ...
        if data_args.task_name is not None:
            ...
            metric = load_metric("../../../pretrain/XLMR+Pretraining/pretrain/accuracy")
        ...
        if data_args.task_name is not None:
            ...
            # res=classification_report(p.label_ids, preds, target_names=target_names)
            # print(res)
            TP = ((preds == p.label_ids) & (preds != 0)).astype(int).sum().item()
            # if p.label_ids.count(1)==618:
            # texts=[]
            # import json
            # with open('/data1/gl/project/ner-relation/kbio-xlm/downstream/RE/data/seqcls/pyq/clean_data/total.json','r',encoding='utf-8') as f:
            #     while True:
            #         line =f.readline()
            #         if not line:
            #             break
            ...
            #         texts.append(line)
            # wf=open('/data1/gl/project/ner-relation/kbio-xlm/downstream/RE/src/runs/pyq/hfl/chinese-roberta-wwm-ext-3e-5-10-52-16/total.json','w',encoding='utf-8')
            # for item,pred in zip(texts,preds):
            #     pred=target_names[pred]
            #     item['label']=target_names[item['label']]
