...
class Token:
    def __init__(self, text,pos_tag=None,related_token=None,related_dep=None, idx=None):
...
class ConllDataset(torch.utils.data.Dataset):
    def __init__(self, name,args, filepath=None):
        self.name = name
        self.sentences = []
        if filepath is not None:
            ...
            with open(filepath, encoding="utf-8") as f:
                for line in f:
                    if line.isspace():
                        if len(tokens) > 0:
                            if not (tokens[0]=='-DOCSTART-' and len(tokens)==1):
                                self.sentences.append(ConllDataset.create_sentence("%s-%s-%d" % (name,pmid,idx), tokens, tags,args))
                                idx += 1
                        tokens, tags = [], []
                    else:
                        sp = line.strip().split('\t')
            ...
            print("Load %s sentences from %s" % (len(self.sentences), filepath))

    @staticmethod
    def create_sentence(idx, tokens, tags, args,label_type="gold"):
        sentence = Sentence(idx=idx)
        tokens_num=0
        for t, tag in zip(tokens, tags):
            token = Token(t)
            tokens_num+=len(args.tokenizer.tokenize(t))
            if tokens_num>args.max_position_embeddings-2:
                break
    ...
    with open(path,'r',encoding='utf-8') as f:
        while True:
            ...
            if not con:
                break
            con=json.loads(con)
            dictory[con['doc_key']]=con['sentences']
...
class ConllCorpus(Corpus):
    def __init__(self, args,use_multi_mask):
        train = ConllDataset("%s-train" % args.task_name,args, args.train_filepath)
        dev = ConllDataset("%s-dev" % args.task_name,args, args.dev_filepath) if args.dev_filepath is not None else None
        test = ConllDataset("%s-test" % args.task_name,args, args.test_filepath) if args.test_filepath is not None else None
        super(ConllCorpus, self).__init__(train, dev, test, name=args.task_name)
