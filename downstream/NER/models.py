...
from transformers import BertTokenizer, BertConfig, BertModel,XLMRobertaTokenizerFast,RobertaConfig,RobertaModel
...
class TransformerEncoder(torch.nn.Module):
    def __init__(self, args, devive):
        ...
        # self.tokenizer = BertTokenizer.from_pretrained(args.pretrained_dir,do_lower_case=args.do_lower_case)
        if args.type=='bert':
            self.tokenizer=BertTokenizer.from_pretrained(args.pretrained_dir,do_lower_case=args.do_lower_case)
        else:
            self.tokenizer=XLMRobertaTokenizerFast.from_pretrained(args.pretrained_dir,do_lower_case=args.do_lower_case)
        if args.type=='bert':
            self.config = BertConfig.from_pretrained(args.pretrained_dir, output_hidden_states=True)
            self.config.max_position_embeddings=args.max_position_embeddings
    ...
    def convert_token_to_subtokens(self, sentence):
        ...
        for t in sentence:
            ...
            tokens_num+=len(subtokens)
            if tokens_num>self.max_position_embeddings-2:
                break
    ...
    def _get_transformer_embeddings(self, sentences, fine_tune=False):
        gradient_context = torch.enable_grad() if fine_tune else torch.no_grad()
        with gradient_context:
            inputs, lengths = [], []
            for s in sentences:
                if s.tokens_indices is None:
                    sub_tokens_ids=self.convert_token_to_subtokens(s)
                    if self.args.type!='bert':
                        s.tokens_indices = [self.tokenizer.bos_token_id] + sub_tokens_ids + [self.tokenizer.eos_token_id]
                    else:
                        s.tokens_indices = [self.tokenizer.cls_token_id] + sub_tokens_ids + [self.tokenizer.sep_token_id]
    ...
    def forward(self, sentences: List[Sentence]):
        ...
        return outputs,torch.tensor(masks).to(self.device)
...
class LinearCRF(torch.nn.Module):
    ...
    def get_focal_mask(self,focal_mask,score):
        masks=torch.zeros_like(score)
        for i,(m,s) in enumerate(zip(focal_mask,score)):
            if m:
                if s>0:
                    masks[i]=0.5
                else:
