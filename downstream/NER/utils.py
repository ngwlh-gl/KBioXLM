...
def get_category2mentions(dataset,args):
    mentions = []
...
def generate_sentences_by_replace_mention(sentence, category2mentions, args):
    ...
    for i in range(num_generated_samples):
        ...
        generated_sentence.bcls=sentence.bcls
        generated_sentence.mcls=sentence.mcls
        for j, token in enumerate(sentence.tokens):
            label = token.get_label("gold")
            if label == "O" or label=='B-X':
                # tokens_num+=len(args.tokenizer.tokenize(token.text))
                # if tokens_num>max_position_embeddings-2:
                #     break
                generated_token = Token(token.text,token.pos_tag,token.related_token,token.related_dep)
                generated_token.set_label("gold", label)
                generated_token.set_label('mtcls','O')
                generated_sentence.add_token(generated_token)
            elif label[0] == "B":
                category = label[2:]
                # if label.endswith('EXP'):
                #     replace_ratio=1
                if np.random.binomial(1, replace_ratio, 1)[0]:
                    ...
                    generated_token = Token(replaced_mention[0],'[UNK]','[UNK]','[UNK]','[UNK]')
                    generated_token.set_label("gold", "B-%s" % category)
                    if len(replaced_mention)>1:
                        generated_token.set_label('mtcls','B-Multi')
                    else:
                        generated_token.set_label('mtcls','B-Single')
                    generated_sentence.add_token(generated_token)
                    for k,t in enumerate(replaced_mention[1:]):
                        
                        generated_token = Token(t,'[UNK]','[UNK]','[UNK]','[UNK]')
                        generated_token.set_label("gold", "I-%s" % category)
                        generated_token.set_label('mtcls','I-Multi')
                        generated_sentence.add_token(generated_token)
                    ...
                    generated_token = Token(token.text,token.pos_tag,token.related_token,token.related_dep)
                    generated_token.set_label("gold", "B-%s" % category)
                    # generated_token.set_label('mtcls',token.get_label('mtcls'))
                    generated_sentence.add_token(generated_token)
                    next = j + 1
                    while next < len(sentence) and sentence[next].get_label("gold")[0] == "I":
                        next_token = sentence[next]
                        
                        generated_token = Token(next_token.text,next_token.pos_tag,next_token.related_token,next_token.related_dep)
                        # generated_token.set_label('mtcls',next_token.get_label('mtcls'))
                        generated_token.set_label("gold", "I-%s" % category)
                        generated_sentence.add_token(generated_token)
                        next += 1
                    # if not flag:
...
def _shuffle_within_segments(tags, replace_ratio):
    ...
    while start < len(segments) and end < len(segments):
        ...
        if len(segment) > 1 and tags[start]!='B-X' and np.random.binomial(1, replace_ratio, 1)[0] == 1:
...
def generate_sentences_by_shuffle_within_segments(sentence, args):
    replace_ratio=args.replace_ratio,
    num_generated_samples=args.num_generated_samples
    max_position_embeddings=args.max_position_embeddings
    sentences = []
    for i in range(num_generated_samples):
        ...
        for i, tag in zip(shuffled_idx, tags):
            generated_token = Token(sentence[i].text,sentence[i].pos_tag,sentence[i].related_token,sentence[i].related_dep)
            generated_token.set_label("gold", tag)
            # generated_token.set_label('mtcls',sentence[i].get_label('mtcls'))
            generated_sentence.add_token(generated_token)
        sentences.append(generated_sentence)
    return sentences


def get_label2tokens(dataset, p_power,args):
    token_freq = {}
    for sentence in dataset:
        for token in sentence:
...
def generate_sentences_by_replace_token(sentence, label2tokens, args):
    replace_ratio=args.replace_ratio,
    num_generated_samples=args.num_generated_samples
    max_position_embeddings=args.max_position_embeddings
    sentences = []
    for i in range(num_generated_samples):
        ...
        for mask, token in zip(masks, sentence.tokens):
            label = token.get_label("gold")
            # if label!='O' and label.endswith('EXP'):
            #     mask=1
            if mask == 0 or token.text.lower() in STOPWORDS or label=='B-X':
                # tokens_num+=len(args.tokenizer.tokenize(token.text))
                # if tokens_num>max_position_embeddings-2:
                #     break
                generated_token = Token(token.text,token.pos_tag,token.related_token,token.related_dep)
                ...
                replace_token=label2tokens[label][0][random_idx]
                replace_token_pos='[UNK]'
                # tokens_num+=len(args.tokenizer.tokenize(replace_token))
                # if tokens_num>max_position_embeddings-2:
                #     break
                generated_token = Token(replace_token,replace_token_pos,'[UNK]','[UNK]','[UNK]')
            generated_token.set_label("gold", label)
            # generated_token.set_label('mtcls',token.get_label('mtcls'))
            generated_sentence.add_token(generated_token)
...
def generate_sentences_by_synonym_replacement(sentence, args):
    replace_ratio=args.replace_ratio,
    num_generated_samples=args.num_generated_samples
    max_position_embeddings=args.max_position_embeddings
    sentences = []
    for i in range(num_generated_samples):
        ...
        for mask, token in zip(masks, sentence.tokens):
            ...
            if mask == 0 or token.text.lower() in STOPWORDS or label=='B-X':
                ...
                if len(synonyms) == 0:
                    generated_token = Token(token.text,'[UNK]','[UNK]','[UNK]','[UNK]')
                    generated_token.set_label("gold", label)
                    # generated_token.set_label('mtcls',token.get_label('mtcls'))
                    generated_sentence.add_token(generated_token)
                    continue
                ...
                for s_i, s_token in enumerate(synonym.split()):
                    ...
                    generated_token = Token(s_token,'[UNK]','[UNK]','[UNK]','[UNK]')
