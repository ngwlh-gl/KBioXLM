...
def get_zh_medical_entity():
    dic={}
    with open('../../XLMR+Pretraining/process_data/utils_data/zh_to_en.json') as f:
        cons=f.readlines()
    bar=tqdm(cons)
    for line in bar:
...
def filter_medical_article(text_lst,medical_dict):
    ...
    for span,candidate in candidates.items():
        try:
            ...
            entities[span]={'zh':candidate.strip(),'en':v['en'],'label':v['label'],'id':v['id']}
        except:
            pass
...
def random_replace_entity(text_lst,entities,num):
    ...
    for i,(k,v) in enumerate(entities.items()):
        if i in random_choice:
            ...
            en_entity=v['en']
            zh_entity=v['zh']
            assert zh_entity==''.join(text_lst[span[0]:span[1]]).strip()
            # res=res.strip()
...
def entity_knowledge(in_file,out_file):
    medical_entity=get_zh_medical_entity()
...
def get_pair_entity(text_lst,entities,medical_entity,entity_to_rel):
    ...
    for k1,v1 in entities.items():
        ...
        for k2,v2 in entities.items():
            ...
            try:
                ...
                en_rich_con.update([' </s> '+v1['en']+' <unk> '+rel['en_rel']+' <unk> '+v2['en']])
                zh_rich_con.update([' </s> '+v1['zh']+' <unk> '+rel['zh_rel']+' <unk> '+v2['zh']])
                num=random.random()
                if num<0.5:
                    mix_rich_con.update([' </s> '+v1['en']+' <unk> '+rel['en_rel']+' <unk> '+v2['en']])
                else:
                    mix_rich_con.update([' </s> '+v1['zh']+' <unk> '+rel['zh_rel']+' <unk> '+v2['zh']])
                # print(rel)
            except:
                pass
        ...
        if text.strip():
            ...
            while i<len(texts):
                ...
                if len(sub_text)>=256:
                    ...
                    if len(entities)>0:
                        en_rich_con,zh_rich_con,mix_rich_con=get_pair_entity(text_lst,entities,medical_entity,entity_to_rel)
                        if en_rich_con:
...
if __name__=="__main__":
    in_file='./data/chinese_examples.json'
    out_file='./data/chinese_entity_text.json'
    entity_knowledge(in_file,out_file)
    out_file='./data/chinese_fact_text.json'
