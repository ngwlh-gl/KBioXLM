...
class DataCollatorForWWM_entity(DataCollatorForWholeWordMask):
    def torch_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        if isinstance(examples[0], Mapping):
            ...
            for e in examples:
                ...
                try:
                    if e['labels']:
                        cls_labels.append(e['labels'])
                    else:
                        cls_labels.append(-100)
                except:
                    cls_labels.append(-100)
        else:
            input_ids = examples
        ...
        for e in examples:
            ...
            entities=e['entities']
            if lang=='en':
                mask_labels.append(self._whole_entity_mask_en(e["input_ids"],entities))
            else:
                mask_labels.append(self._whole_entity_mask_zh(e["input_ids"],entities))
        batch_mask = _torch_collate_batch(mask_labels, self.tokenizer, pad_to_multiple_of=self.pad_to_multiple_of)
        # inputs, labels = self.torch_mask_tokens(batch_input, batch_mask,batch_labels)
        inputs, labels = self.torch_mask_tokens(batch_input, batch_mask,batch_labels)

        # input和labels都要投射到student的词表上
        # student_inputs,student_labels=self.convert_tea_to_stu(inputs,labels)
        # return {"input_ids": inputs, "attention_mask":batch_attention_mask, "labels": labels, 'student_input_ids':student_inputs, 'student_labels':student_labels}
        return {"input_ids": inputs, "attention_mask":batch_attention_mask, "labels": labels,'cls_labels':torch.tensor(cls_labels)}
    ...
    def _whole_entity_mask_en(self, input_tokens: List[str],entities, max_predictions=512):
        """
        ...
        for entity in entities:
            if entity['span'][0]<max_predictions:
                cand_indexes.append(entity['span'])
    ...
    def _whole_entity_mask_zh(self, input_tokens: List[str],entities, max_predictions=512):
        """
        Get 0/1 labels for masked tokens with whole word mask proxy
        """
